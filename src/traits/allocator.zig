...
pub const AllocatorTrait = struct {
    ...
    pub fn alloc(self: *Self, len: usize, alignment: Alignment, ret_addr: usize) ?[*]u8 {
        _ = self;
        _ = len;
        _ = alignment;
        _ = ret_addr;
        @compileError("Not an availd implement.");
    }
    ...
    pub fn remap(self: *Self, memory: []u8, alignment: Alignment, new_len: usize, ret_addr: usize) ?[*]u8 {
        _ = self;
        _ = memory;
        _ = alignment;
        _ = new_len;
...
pub fn Allocator(comptime A: type) type {
    ...
    return struct {
        ...
        pub fn create(self: *Self, comptime T: type) Error!*T {
            ...
            const ptr: *T = @ptrCast(try self.allocBytesWithAlignment(.of(T), @sizeOf(T), @returnAddress()));
            return ptr;
        }
